# -*- coding: utf-8 -*-
"""
Created on Wed May 31 08:46:49 2023
...
def propriedades(ponto):
    ...
    output_var = [
        variable for variable in variaveis if variable not in input_var]
    outputs = PropsSI(output_var, input_var[0], ponto[input_var[0]],
                      input_var[1], ponto[input_var[1]], ponto['refrigerant'])
...
def COP_Duplo_Estagio_Compress(cycle_inputs):

    P_ETB = PropsSI("P", "Q", 1, "T",
                    cycle_inputs['t_internal_f'], cycle_inputs['refrigerant'])

    # Pós - Condensador (propriedades do ponto 3)
    point_3 = {'Q': 0, 'T': cycle_inputs['t_cond'] -
               cycle_inputs['subcooling'], 'refrigerant': cycle_inputs['refrigerant']}
    propriedades(point_3)

    # Evaporador da Geladeira
    point_5 = {'Q': cycle_inputs['tit_evap_g'], 'T': cycle_inputs['t_internal_g'],
               'refrigerant': cycle_inputs['refrigerant']}
    propriedades(point_5)

    P_int = point_5['P']

    point_4 = {'HMASS': point_3['HMASS'], 'P': P_int,
               'refrigerant': cycle_inputs['refrigerant']}
    ...
    point_2s = {'SMASS': point_1['SMASS'], 'P': point_3['P'],
                'refrigerant': cycle_inputs['refrigerant']}
    propriedades(point_2s)

    point_2 = {'P': point_3['P'], 'HMASS': point_1['HMASS'] + (
        (point_2s['HMASS'] - point_1['HMASS'])/cycle_inputs['N_isent']), 'refrigerant': cycle_inputs['refrigerant']}
    ...
    point_7 = {'HMASS': point_6['HMASS'], 'P': P_ETB,
               'refrigerant': cycle_inputs['refrigerant']}
    ...
    point_9s = {
        'P': P_int, 'SMASS': point_8['SMASS'], 'refrigerant': cycle_inputs['refrigerant']}
    propriedades(point_9s)

    point_9 = {'P': P_int, 'HMASS': point_8['HMASS'] + (
        point_9s['HMASS'] - point_8['HMASS']), 'refrigerant': cycle_inputs['refrigerant']}
    ...
    m2 = m1*(point_6['HMASS'] - point_9['HMASS']) / \
        (point_5['HMASS'] - point_1['HMASS'])
    # print(m2)
...
COP_Duplo_Estagio_Compress(input_values)
