# -*- coding: utf-8 -*-
"""
Created on Sat Jun 17 13:52:41 2023
...
def func(x,cycle_inputs):
    ...
    return Ef_Ex_Serie(cycle_inputs)
...
def derivada_razao(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_x = Ef_Ex_Serie(cycle_inputs)
    new_cycle_x = Ef_Ex_Serie(cycle_inputs_x)
...
def derivada_titulo(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_y = Ef_Ex_Serie(cycle_inputs)
    cycle_inputs_y = copy.copy(cycle_inputs)
    cycle_inputs_y['variacao_titulo_f'] += delta    
    new_cycle_y = Ef_Ex_Serie(cycle_inputs_y)
...
def SD_titulo(cycle_inputs, delta = 10e-3, tol=10e-05, gama = 0.1):
    ...
    while np.abs(deriv) > tol:
        ...
        if cycle_inputs['variacao_titulo_f'] > 0.3:
            break
...
def derivada_superheating(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_z = Ef_Ex_Serie(cycle_inputs)
    new_cycle_z = Ef_Ex_Serie(cycle_inputs_z)
...
def derivada_subcooling(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_w = Ef_Ex_Serie(cycle_inputs)
    new_cycle_w = Ef_Ex_Serie(cycle_inputs_w)
