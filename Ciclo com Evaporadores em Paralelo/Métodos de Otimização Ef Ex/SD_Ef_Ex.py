# -*- coding: utf-8 -*-
"""
Created on Sat Jun 17 17:51:26 2023
...
def derivada_razao(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_x = Ef_Ex_Paralelo(cycle_inputs)
    new_cycle_x = Ef_Ex_Paralelo(cycle_inputs_x)
...
def SD_razao(cycle_inputs, delta = 10e-3, tol=10e-5, gama = 5):
...
def derivada_titulo(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_y = Ef_Ex_Paralelo(cycle_inputs)
    cycle_inputs_y = copy.copy(cycle_inputs)
    cycle_inputs_y['tit_evap_f'] += delta    
    new_cycle_y = Ef_Ex_Paralelo(cycle_inputs_y)
...
def SD_titulo(cycle_inputs, delta = 10e-3, tol=10e-04, gama = 1):
    ...
    while np.abs(deriv) > tol:
        ...
        if cycle_inputs['approach_HX'] > 0.65:
            break
...
def derivada_superheating(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_z = Ef_Ex_Paralelo(cycle_inputs)
    new_cycle_z = Ef_Ex_Paralelo(cycle_inputs_z)
...
def SD_superheating(cycle_inputs, delta = 10e-3, tol=10e-6, gama = 5):
...
def derivada_subcooling(cycle_inputs,delta = 10e-3):
    ...
    current_cycle_w = Ef_Ex_Paralelo(cycle_inputs)
    new_cycle_w = Ef_Ex_Paralelo(cycle_inputs_w)
...
def SD_subcooling(cycle_inputs, delta = 10e-3, tol=10e-6, gama = 0.1):
